...
def createAccount(username, password):
	...
	script = f"CREATE TABLE {username} (fileid integer, filename text, lastSavePoint text, hearts integer, arrows integer, coins integer, bosses_killed integer, items text, map_discovered text, settings text)"
	# settings layout " windowsize, difficulty, volume, music, sfx "
	cur.execute(script)
	# create 3 save files
	for ID in range(3):
		script = f"INSERT INTO {username} VALUES ({ID}, 'Empty', 'X,Y', 100, 25, 0, 0, '', '', '900x600,hero,100,100,100')"
		cur.execute(script)
