...
for weapon in weapons:
	...
	player.animations[f'{weapon}/moving'], images =engine.animation.load_weapon_animation(f'sprites/herakles/{weapon}/moving', [5,5])
	player.animation_images.update(images)
	player.animations[f'{weapon}/attack'], images =engine.animation.load_weapon_animation(f'sprites/herakles/{weapon}/attack', [10,5,10])
	player.animation_images.update(images)
# crawl animation
...
# loads game map
gamemap = engine.map('hell', 'A', hell_tiles)
...
while not done:
	...
	for arrow in arrows:
		...
		if arrow.tile_collisions(gamemap.tiles_list):
			arrows.remove(arrow)
			del arrow
