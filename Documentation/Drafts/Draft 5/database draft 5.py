...
def createAccount(username, password):
	...
	script = "CREATE TABLE "+username+" (fileid integer, filename text, lastSavePoint text, hearts integer, arrows integer, coins integer, bosses_killed integer, items text, map_discovered text, settings text)"
	cur.execute(script)
	# create 3 save files
	for ID in range(3):
		script = "INSERT INTO "+username+f" VALUES ({ID}, 'Empty', 'X,Y', 100, 0, 0, 0, '', '', '')"
		cur.execute(script)
...
def deleteSaveFile(fileID):
	...
	script = "UPDATE "+username+" SET filename='Empty', lastSavePoint='X,Y', hearts=100, arrows=0, coins=0, bosses_killed=0, items='', map_discovered='', settings='' WHERE fileid = "+str(fileID)
	cur.execute(script)
