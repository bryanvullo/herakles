...
class settings_menu:
	...
	def save(self, volume, music, sfx):
		...
		if self.window_size != 'fullscreen':
			sizes = self.window_size.split('x')
			sizes[0] = int(sizes[0])
			sizes[1] = int(sizes[1])
			credentials.window_size = sizes
		else:
			credentials.window_size = 'fullscreen'
	...
	def menu(self):
		...
		size3_button=Button(main, text='fullscreen', 
			command=lambda: self.set_size('fullscreen'))
